...
struct ContentView: View {
            ...
            .onChange(of: referenceItem) {
                Task {
                    if let data = try? await referenceItem?.loadTransferable(type: Data.self),
                       let image = UIImage(data: data) {
                        referenceImage = image; resultImage = nil; processingTime = nil
                    }
                }
            }
        }
    ...
    private func loadSample(src: String, ref: String) {
        if let s = Bundle.main.path(forResource: src, ofType: "png"),
           let r = Bundle.main.path(forResource: ref, ofType: "png"),
           let si = UIImage(contentsOfFile: s), let ri = UIImage(contentsOfFile: r) {
            sourceImage = si; referenceImage = ri; resultImage = nil; processingTime = nil
        }
    }

    private func runTransfer() {
        guard let src = sourceImage, let ref = referenceImage else { return }
        isProcessing = true; status = "Applying makeup..."; resultImage = nil; processingTime = nil
        Task {
            ...
            do {
                ...
                await MainActor.run {
                    resultImage = result; processingTime = elapsed; status = ""; isProcessing = false
                }
            } catch {
                await MainActor.run { status = "Error: \(error.localizedDescription)"; isProcessing = false }
            }
        }
    }
}
