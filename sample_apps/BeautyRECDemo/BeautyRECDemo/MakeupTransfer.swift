...
enum MakeupTransfer {
    ...
    static func transfer(source: UIImage, reference: UIImage) async throws -> UIImage {
        ...
        // 1. Parse both faces to get masks
        let parserModel = try loadModel(containing: "FaceParser")
        let srcMask = try await parseFace(model: parserModel, buffer: srcBuf)
        let refMask = try await parseFace(model: parserModel, buffer: refBuf)
    ...
    private static func parseFace(model: MLModel, buffer: CVPixelBuffer) async throws -> MLMultiArray {
    ...
    private static func multiArrayToImage(_ array: MLMultiArray, width: Int, height: Int) -> UIImage {
        ...
        if array.dataType == .float16 {
            ...
            for y in 0..<height {
                for x in 0..<width {
                    let idx = (y * width + x) * 4
                    let r = Float(ptr[0 * count + y * width + x])
                    let g = Float(ptr[1 * count + y * width + x])
                    let b = Float(ptr[2 * count + y * width + x])
                    pixels[idx]     = UInt8(clamping: Int(min(max(r, 0), 1) * 255))
                    pixels[idx + 1] = UInt8(clamping: Int(min(max(g, 0), 1) * 255))
                    pixels[idx + 2] = UInt8(clamping: Int(min(max(b, 0), 1) * 255))
                }
            }
        } else {
            ...
            for y in 0..<height {
                for x in 0..<width {
                    let idx = (y * width + x) * 4
                    let r = ptr[0 * count + y * width + x]
                    let g = ptr[1 * count + y * width + x]
                    let b = ptr[2 * count + y * width + x]
                    pixels[idx]     = UInt8(clamping: Int(min(max(r, 0), 1) * 255))
                    pixels[idx + 1] = UInt8(clamping: Int(min(max(g, 0), 1) * 255))
                    pixels[idx + 2] = UInt8(clamping: Int(min(max(b, 0), 1) * 255))
                }
